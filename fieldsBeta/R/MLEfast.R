...
MLE.objective.fn <- function(ltheta, info, value = TRUE) {
    ...
    temp.fn <- function(llam, info) {
        ...
        N2 <- length(u2.MS)
        # MLE of sigma
        sigma.MLE <- (sum((u2.MS)/(lam.temp + D2)))/N2
        # ln determinant
        lnDetCov <- sum(log(lam.temp + D2))
        -1 * M * (-N2/2 - log(2 * pi) * (N2/2) - (N2/2) * log(sigma.MLE) - 
            (1/2) * lnDetCov)
    }
    ...
    sigma.MLE <- (sum((u2.MS)/(lambda.MLE + D2)))/N2
    tau.MLE <- sqrt(lambda.MLE * sigma.MLE)
    trA <- sum(D2/(lambda.MLE + D2))
    pars <- c(sigma.MLE, theta, tau.MLE, trA)
    names(pars) <- c("sigma", "theta", "tau", "trA")
    if (value) {
...
MLE.Matern.fastOLD <- function(x, y, smoothness, theta.grid = NULL, 
    ...
    theta.MLE <- exp(out$x)
    REML <- -out$fmin
    # one final call with the theta.MLE value to recover MLEs for sigma and tau
