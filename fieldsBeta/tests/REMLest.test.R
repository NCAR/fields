...
# and only theta (range), sill ( sigma) and nugget (tau2) are considered. 
##########################################################################
...
# testing REML formula as used in gcv.Krig
 ...
 loglmvn <- function(pars, nu, x, y) {
        ...
        lsig2 = pars[3]
        d <- rdist(x, x)
        A <- exp(lsigma)*(Matern(d, range = exp(ltheta), 
            smoothness = nu) + exp(lsig2)/exp(lsigma) * diag(N))
        A <- t(Q2) %*% A %*% Q2
        A <- chol(A)
 ...
 logProfilemvn <- function(lambda, theta, nu, x, y) {
   ...
   w = backsolve(A, ys, transpose = TRUE)
   sigma.MLE<- sum( w^2)/N2
   REMLLike<- -1 * (-N2/2 - log(2 * pi) * (N2/2) - (N2/2) * log(sigma.MLE) - 
                   (1/2) * lnDetCov)
   return( REMLLike)
 }   
 
out<- Krig( x,y, Covariance="Matern", smoothness= nu, theta= 2.0, method="REML"  )
pars<- c(log( out$sigma.MLE), log( 2.0), log( out$tauHat.MLE^2) )
...
##D test.for.zero( hold1$pars[1], hold2$pars[1], tol=2e-5, tag="check REML sigma")
##D test.for.zero( hold1$pars[2], hold2$pars[2], tol=2e-5, tag="check REML theta")
##D test.for.zero( hold1$pars[3], hold2$pars[3], tol=5e-6, tag=" check REML tau2")

hold3<- MaternQR.test( x,y,nu)
hold4<- MaternQRProfile.test( x,y,nu)
test.for.zero( hold3$pars[1], hold4$pars[1], tol=1e-3, tag="check REML sigma")
test.for.zero( hold3$pars[2], hold4$pars[2], tol=1e-3, tag="check REML theta")
test.for.zero( hold3$pars[3], hold4$pars[3], tol=.0002, tag=" check REML tau2")
...
#hold6<- spatialProcess( x,y, smoothness=nu, theta= hold4$pars[2], REML=TRUE)
#test.for.zero( hold6$lnProfileREML.FULL ,l2, tag="Profile likelihoods from Krig and spatialProcess")
