...
The `fastTps` function is actually a wrapper for the `mKrig` function with the compactly supported Wendland covariance function. The taper range `theta` and the the polynomial power for the Wendland rbf `p` must be chosen. The defaults are `theta = 1` and `p = 2m-d` where `m` is chosen such that `p` is positive.
...
fastTps( RMprecip$x,RMprecip$y,m=2,lambda= 1e-2, theta=3.0) -> out1
fastTps( RMprecip$x,RMprecip$y,m=2,lambda= 1e-1, theta=3.0) -> out2
fastTps( RMprecip$x,RMprecip$y,m=2,lambda= 1, theta=3.0) -> out3
```
...
Note that setting `theta` to 3 degrees is a very generous taper range. Use some trial `theta` values with `rdist.nearest` to determine a a useful taper (see the examples in `mKrig`). Some empirical studies suggest that in the interpolation case in 2d, the taper should be large enough so there are about 20 non-zero nearest neighbors for every location.

Now we use great circle distance for this smooth. Note that when `lon.lat=TRUE`, `theta` for the taper support is automatically converted to degrees.

```{r, results='hide'}
fastTps( RMprecip$x,RMprecip$y,m=2,lambda= 1e-2,lon.lat=TRUE, theta=300) -> out4
out3$eff.df
out4$eff.df 
```
