...
mKrigMLEJointOld <- function(x, y, weights = rep(1, nrow(x)),  Z = NULL,
                            mKrig.args = NULL,
                                 na.rm = TRUE,
  ...
  # (most of the time one would want to optimize over lambda parameter)
    ...
    mKrig.args<- c( mKrig.args, list(  find.trA = find.trA))
  if( verbose){
...
##########################################################################  
  if(  is.null(cov.params.start) ){
    #########################################################
    ### fast return if there is nothing to do but just evaluate
    ### at the given parameters 
    #########################################################
    if( verbose){
      cat("fast return in MLEJoint", fill=TRUE)
    }
    fastObject <- do.call("mKrig", c(mKrig.args, cov.args) )
    lnLike.eval <-  NULL
    optimResults <- NULL
# there should always be these 3 parameters in the model
# sigma and rho are not really MLEs but the maximizer given the lambda value
    sigma.MLE    <- fastObject$sigma.MLE
    rho.MLE      <- fastObject$rho.MLE
    lambda.MLE   <- NA
    ...
    optim.counts<- c( NA, NA) # optim is not called
    parOptimum <- NULL # no parameters were optimized
    # NOTE there may be other fixed covariance parameters not listed here 
    # but it is hard to extract their names from other covariance function arguments. 
    # but the user should know what these are in the call to this function 
    #parNames<- c( "lambda", "theta")
    #parOptimum<- c(cov.args$lambda, cov.args$theta)
    #names( parOptimum)<- parNames
    if( verbose){
      cat("fast return parOptimum, optim.value",  fill=TRUE)
      cat(parOptimum, optim.value,fill=TRUE)
    }
    #########################################################  
  }
  else{
...
# 
  ...
  print(cov.params.start )
  cat("initial parameter starts (possibly after transformation such as log: ", fill=TRUE)
  cat(init.start, fill=TRUE)
  }
...
# optimize over (some) covariance parameters and possibly lambda
  ...
  optim.value  <- optimResults$value
  
  if( !is.null( parOptimum["lambda"] )){
    lambdaModel<- parOptimum["lambda"]
  ...
  sigma.MLE<- lnLike.eval[ind,"sigma.MLE"]
  rho.MLE  <- lnLike.eval[ind,"rho.MLE"]
  eff.df  <- NA
}
##########################################################################
###### end main if else block
########################################################################## 
  if( !is.null(parOptimum)){
  ...
  names(summary) <- c(nameCriterion, parNames, 
    ...
    names(summary) <- c(nameCriterion,  
                        "sigmaMLE", "rhoMLE","funEval", "gradEval")
  }
  
  if( verbose){
  cat( "final summary from mKrigMLEJoint", fill=TRUE)  
    print( summary) 
  }
