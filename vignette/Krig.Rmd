...
vls.matern <- function(vec){ # vec = (rho,theta,nu,sigmasq)
...
fit2 <- Krig(grd.CO,y.CO,Covariance="Matern",theta=theta,rho=rho,smoothness=nu,sigma2=sigmasq)
#fit2 <- mKrig(grd,y,cov.function="stationary.cov",Covariance="Matern",theta=theta)
```
...
Performing a spatial prediction via `Krig` is easy, but first we need to propose a range parameter `theta`. To get an estimate of `theta`, we determine sample autocorrelation of matrix `ozone2$y` (which allows us to use all 89 days of measurement), and then plot the correlation versus distance between locations. The sample correlation is around zero when $d=350$, so we will use the range `theta=350` with `Krig`.
...
Now it's time to `Krig` with our estimate `theta=350`. Note that visualizing a `Krig` object via `surface` only displays the spatial part of the model $g(\cdot)$ and the low order spatial polynomial $P(\cdot)$ (i.e. it excludes the `Z ` part of the linear trend). To get a plot of the full estimate involving a covariate `Z`, we need to use `predictSurface.Krig`. Below are few different ways to reach the same result (no `Z` covariate yet).

```{r}
out <- Krig( x.ozone, y.ozone, theta=350,Covariance="Matern",Distance="rdist.earth")
surface(out)
```
...
Here is the code for the final image in the Geostatistics section. We use the same naming scheme as in that example. Note that the `Krig` function only requires the `theta` parameter, and it will estimate the unspecified parameters by REML or GCV.
...
fit1 <- Krig(grd.CO,y.CO,Covariance="Matern",Z=z.CO,theta=350)
fit2 <- Krig(grd.CO,y.CO,Covariance="Matern",Z=z.CO,theta=350,rho=rho,sigma2=sigmasq,smoothness=nu)
...
surface(pred1,main="Only fix theta",zlim=c(0,23))
US(add=TRUE)
