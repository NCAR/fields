...
If the user creates a function (e.g. `spherical(d,theta)`) that takes distances as an argument, they can use this in `stationary.cov`.

Suppose we want to create the isotropic spherical covariance, which is compactly supported and only depends on the  parameter distance $r = \| \mathbf x - \mathbf x'\|$ and the range $\theta$:
\[
C(r) = \left( 1 - \frac{3r}{2 \theta}  + \frac{1}{2} \left( \frac{r}{\theta} \right)^3 \right) \cdot \mathbf{1}_{\{0 \le r < \theta \} }
\]

Here, we show two alternatives to constructing and using a covariance function in `fields`.

```{r}
   spherical <-  function (x1, x2=NULL, theta=1, C=NA, Distance="rdist" ) {
       if (is.null(x2)) {x2 <- x1}
       if (Distance == "rdist.earth"){
         r <- rdist.earth(x1,x2)
       }else {r <- rdist(x1,x2) }
       if (is.na(C[1]) ) {
           return( ifelse(r<theta , 1 - 3*r / (2*theta) + (1/2) * (r/theta)^3,0))
       }
       if (!is.na(C[1])) {
           return( ifelse(r<theta , 1 - 3*r / (2*theta) + (1/2) * (r/theta)^3,0) %*% C)
       }
   }


spherical.d <- function(d, theta=1)
{
  return( ifelse(d<theta , 1 - 3*d / (2*theta) + (1/2) * (d/theta)^3,0))
}
```
...
fit <- Krig(grd,y, cov.function = "spherical",theta=60, Distance="rdist.earth")
fit2 <- Krig(grd,y,Covariance = "spherical.d", theta=60, Distance="rdist.earth")
fit3 <- Krig(grd,y,Covariance="Matern",theta=60, Distance="rdist.earth")
...
temp <- stationary.taper.cov( x[1:3,],x[1:10,], theta=1.5, Taper.args= list(k=2,theta=2.0,dimension=2) )
temp2<- Exp.cov( x[1:3,],x[1:10,], theta=1.5) * Wendland(rdist(x[1:3,],x[1:10,]),theta= 2.0, k=2, dimension=2)
...
spatialProcess(x,y, cov.function = "stationary.taper.cov", theta=1.5,cov.args = 
       list(Taper.args= list(k=2, dimension=2,theta=2.0) )) -> out2
spatialProcess(x,y, theta= 1.5)-> out
set.panel(1,2)
