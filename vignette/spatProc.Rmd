...
Next we look at the summary of the model parameters. The optimization search is automated in `fields`, but all of the information is easily accessible for the user. Recall the range is $\theta$, the nugget is $\sigma^2$, the sill is $\rho$, and we set $\lambda=\sigma^2/\rho$.
...
drape.plot(sim.surf, border=NA, theta = 30, phi= 25,col=topo.colors(100)) -> dp
...
{drape.plot(lookSE, border=NA, theta=160, phi=55,col=topo.colors(100)) -> dp2
title("Drape Plot")
...
<We see that `MLEspatialProcess` estimates `theta` decently well but `sigmaMLE` is far from the truth. Try with more data points! Now we switch to a Matern covariance, then compute the joint MLE of range, smoothness, and lambda. > 
