...
MaternGLS.test <- function(x, y, smoothness = 1.5, 
    ...
    loglmvn <- function(pars, nu, y, x, d) {
        ...
        #  print( pars)
        N <- length(y)
        M <- (exp(lsigma) * Matern(d, range = exp(ltheta), smoothness = nu) + 
            exp(lsig2) * diag(N))
        ...
        if ((abs(lsigma) > 20) | (abs(ltheta) > 10) | (abs(lsig2) > 
            40)) {
...
MaternGLSProfile.test <- function(x, y, smoothness = 1.5, 
    ...
    temp <- optim(init, loglmvn, method = "L-BFGS-B", nu = nu, 
        y = y, x = x, d = d)
    out <- exp(temp$par)
    sigma.MLE <- mKrig(x, y, theta = out[2], Covariance = "Matern", 
...
MaternQR.test <- function(x, y, smoothness = 1.5, 
    ...
    loglmvn <- function(pars, nu, x, y) {
        ...
        if ((abs(lsigma) > 100) | (abs(ltheta) > 100) | (abs(ltheta) > 
            100)) {
            return(ycept + 1000 * sum(abs(abs(pars) - 100)))
        }
        else {
...
MaternQRProfile.test <- function(x, y, smoothness = 1.5, 
    ...
    out3 <- c(out2$sigma.MLE * offset, theta.est, out2$tauHat.MLE^2 * 
        offset)
...
REML.test <- function(x, y, sigma, tau2, theta, nu = 1.5) {
    Tmatrix <- fields.mkpoly(x, 2)
    qr.T <- qr(Tmatrix)
    N <- length(y)
    Q2 <- qr.yq2(qr.T, diag(1, N))
    ys <- t(Q2) %*% y
    N2 <- length(ys)
    A <- (sigma * Matern(rdist(x, x), range = theta, smoothness = nu) + 
        tau2 * diag(1, N))
    A <- t(Q2) %*% A %*% Q2
    Ac <- chol(A)
