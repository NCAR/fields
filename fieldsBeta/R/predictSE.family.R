...
"predictSE.mKrig" <- function(object, xnew = NULL, 
    ...
    sigma <- object$sigmahat
    tau2 <- lambda * sigma
    if (verbose) {
        print(c(lambda, sigma, tau2))
    }
    ...
    #   temp1 <-  sigma*(t0%*% object$Omega %*%t(t0)) -
    #          sigma*predict( object, y= k0, x=x) -
    #          sigma*predict( object, y= k0, x=x, just.fixed=TRUE)
    ...
    temp1 <- sigma * (colSums(t0 * (object$Omega %*% t0)) - colSums((k0) * 
        hold$c) - 2 * colSums(t0 * hold$d))
    # find marginal variances -- trival in the stationary case!
    temp0 <- sigma * do.call(call.name,
